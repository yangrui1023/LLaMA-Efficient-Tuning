...
./s5cmd sync s3://sagemaker-us-west-2-000274595152/anno_data_test/* /opt/ml/input/data_test/
...
torchrun --num_gpus=8  /opt/ml/code/src/train_bash.py \
    ...
    --do_train \
    --do_eval \
    --dataset_dir /opt/ml/input/data_test \
    --dataset announcement_test \
    --template default \
    --finetuning_type lora \
    --lora_target W_pack \
    --output_dir /tmp/large_model_test_out \
    --overwrite_cache \
...
# 4 - 训练后的模型参数，从资源机拷贝到S3
	...
	# 会有小时级别的时间消耗
./s5cmd sync /tmp/large_model_test_out s3://sagemaker-us-west-2-000274595152/anno_train_test_output/$(date +%Y-%m-%d-%H-%M-%S)/
